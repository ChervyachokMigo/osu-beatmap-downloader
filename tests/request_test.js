const search_beatmaps_loop = require("../tools/search_beatmaps_loop");
...
const inc = (obj, prop) => {
	typeof obj[prop] === 'undefined' ? obj[prop] = 1 : obj[prop]++;
}

(async () => {
	//gamemode только цифры
	//gamemode без значения находит с другом порядке, включает лавед

	search_beatmaps_loop({
		//gamemode: '0',
		status: 'ranked',
		nsfw: 'true',
		}, (beatmapsets) => {
			...
			beatmapsets.map( x => {
				x.beatmaps.map ( v => {
					//console.log(v)
					inc(gamemodes, v.mode);
					inc(gamemodes_int, v.mode_int);
					inc(beatmap_ranked, v.ranked);
					
				})
                inc(statuses, x.status);
				inc(ranked, x.ranked);
				inc(nsfw, x.nsfw);
			})
			//console.log(beatmapsets);
			console.log('gamemodes', gamemodes);
			//console.log('gamemodes_int', gamemodes_int);
			console.log('beatmap_ranked', beatmap_ranked);
			console.log('statuses', statuses);
			//console.log('ranked', ranked);
			console.log('nsfw', nsfw);
			//availability: { download_disabled: false, more_information: null },
			process.exit()
			console.log('beatmapsets.length', beatmapsets.length);
		}
	)
})();